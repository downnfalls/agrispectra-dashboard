...
void sendAnalyseResult(int leaf_count, bool harvestable) {
  ...
  // ต้องมี println เสมอ เพื่อให้บอร์ดหลักรับรู้ว่าจบประโยค (\n)
  Serial.println();
}
...
void analyseAndUpload() {
  ...
  uint8_t *full_rgb_buf = (uint8_t *)heap_caps_malloc(
      myPhoto->width * myPhoto->height * 3, MALLOC_CAP_SPIRAM);
  if (!full_rgb_buf) {
  ...
  bool converted =
      fmt2rgb888(myPhoto->buf, myPhoto->len, PIXFORMAT_JPEG, full_rgb_buf);
  if (!converted) {
  ...
  ei_camera_frame_buffer =
      (uint8_t *)heap_caps_malloc(ei_buf_size, MALLOC_CAP_SPIRAM);
  ...
  for (uint32_t i = 0; i < result.bounding_boxes_count; i++) {
    ...
    if (strcmp(bb.label, "leaf") == 0)
      leaf_count++;
    else if (strcmp(bb.label, "plant") == 0)
      plant_count++;
    else if (strcmp(bb.label, "ready") == 0)
      ready_count++;
  }
#endif
  ...
  Serial.printf(
      "[AI] Stats -> Plant: %d, Leaf: %d, Ready: %d | Avg Leaf/Plant: %.2f\n",
  ...
  Serial.printf("[Stage] Analyse Result: Leaf Count: %f, Harvestable: %s\n",
