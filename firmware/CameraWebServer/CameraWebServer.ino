...
// ===========================
// *** เปลี่ยนขา GPIO ให้ตรงกับการต่อวงจรจริงของคุณ ***
...
struct StageConfig {
  ...
  Period periods[10]; // รองรับสูงสุด 10 ช่วงเวลาต่อ Stage
  int periodCount = 0;
  int currentActiveValue = -1; // -1 บังคับให้อัปเดตตอนเปิดเครื่อง
};
...
// หาค่าแสงที่ควรจะเป็น ณ เวลาปัจจุบัน
...
void updateLights(int stageIdx, int intensityPercent) {
  // คำนวณ (ค่าสูงสุดของสี * เปอร์เซ็นต์) / 100
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {
  if (type == WStype_TEXT) {
    ...
    // บังคับเคลียร์นาทีล่าสุดเพื่อให้ฟังก์ชันอัปเดตไฟทำงานทันทีที่รับค่าใหม่
    checkPeriodTimer(); 
  }
}
